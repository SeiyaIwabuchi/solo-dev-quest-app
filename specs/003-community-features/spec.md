...
- 質問投稿中にDevCoinが不足した場合、どうなるか？（トランザクション管理）
- 同じユーザーが同じ質問を短時間に複数回投稿しようとした場合は？（重複投稿防止）
- ベストアンサー選択後に質問者がアカウントを削除した場合、回答者の報酬はどうなるか？
- 外部SNS APIがレート制限に達した場合、タイムラインはどう表示されるか？
- SNSアカウント連携を解除した後、過去のいいね・リツイートはどう扱われるか？
- 不適切なコンテンツ（スパム、誹謗中傷等）を含む質問・回答・コメントの報告と削除はどう処理するか？
- プレミアムプラン決済が失敗した場合（カード期限切れ等）、どのタイミングで特典が停止されるか？
- 大量の質問が投稿された場合、検索・フィルタリングのパフォーマンスは保たれるか？
- オフライン時に質問・回答を閲覧した内容はキャッシュされるか？
...
- **FR-014**: システムは、不適切なコンテンツ（スパム・誹謗中傷等）を報告する機能を提供し、管理者による削除を可能にしなければならない
- **FR-015**: システムは、同一ユーザーによる短時間での重複質問投稿を検知し、制限しなければならない（例：同じ質問は5分以内に1回まで）

### Key Entities

- **Question（質問）**: タイトル、本文、コード例、投稿者ID、投稿日時、カテゴリタグ、回答数、閲覧数
- **Answer（回答）**: 本文、回答者ID、投稿日時、質問ID、ベストアンサーフラグ、評価スコア（役立った数/役立たなかった数）
- **Comment（コメント）**: 本文、投稿者ID、投稿日時、対象ID（質問ID or 回答ID）、テンプレート種別
- **HashtagPost（ハッシュタグ投稿）**: SNS種別（X/Threads/Instagram）、投稿ID、投稿者名、アイコンURL、本文、画像/動画URL、投稿日時、元投稿URL
- **SNSInteraction（SNSインタラクション）**: アクション種別（いいね/リツイート/コメント）、実行ユーザーID、対象投稿ID、実行日時
- **PremiumSubscription（プレミアムサブスクリプション）**: ユーザーID、プラン種別（プレミアム/スーパープレミアム）、開始日、次回更新日、ステータス（有効/キャンセル済み/期限切れ）
...
- **NOTE-001**: ハッシュタグタイムライン機能は外部SNS APIの仕様変更リスクがあるため、PoC（Proof of Concept）検証を優先実施し、技術的実現可能性を確認する必要がある
- **NOTE-002**: 不適切コンテンツのモデレーションは運用負荷が高いため、初期はユーザー報告ベースで対応し、将来的にAI自動検知を導入予定
