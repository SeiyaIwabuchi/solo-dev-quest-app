...
2. **Given** 質問が投稿されている、**When** 他のユーザーが質問一覧を閲覧する、**Then** 最新の質問から順に表示され、タイトル・投稿者・投稿日時・回答数・カテゴリタグが確認できる
3. **Given** ユーザーがDevCoinを所持していない、**When** 質問を投稿しようとする、**Then** 「質問の投稿には10 DevCoinが必要です」というメッセージが表示され、「DevCoinを獲得する」（タスク完了画面へ遷移）および「DevCoinを購入する」（課金画面へ遷移）ボタンが表示される
...
1. **Given** 未回答の質問が存在する、**When** ユーザーが回答を投稿する、**Then** 回答が公開され、「回答を投稿しました。5 DevCoinを獲得！」という成功メッセージが表示され、回答者に5 DevCoin（無料）が付与される
2. **Given** 質問に複数の回答が投稿されている、**When** 質問者が最も役立った回答を「ベストアンサー」として選択する、**Then** その回答者に追加で15 DevCoin（無料）が付与され、選択された回答には緑色の「ベストアンサー」バッジが表示され、回答カード全体の背景色が薄緑色にハイライトされる
...
5. **Given** 回答投稿中にネットワークエラーが発生した、**When** 投稿処理が失敗する、**Then** 「回答の投稿に失敗しました。ネットワーク接続を確認してください」というエラーメッセージが表示され、入力内容は保持される
...
5. **Given** ハッシュタグ投稿が1件も存在しない、**When** タイムラインを閲覧する、**Then** 「まだ投稿がありません。#個人開発チャレンジ のハッシュタグをつけてSNSに投稿してみましょう！」というメッセージが表示される
...
4. **Given** SNSアカウント未連携、**When** いいね・リツイート・コメントを試みる、**Then** 「この機能を使用するにはSNSアカウントの連携が必要です」というメッセージと、「[SNS名]を連携する」ボタン、「後で」ボタンを含むダイアログが画面中央に表示される
...
1. **Given** 質問または回答を閲覧している、**When** ユーザーが「コメント」ボタンを押す、**Then** コメント入力欄と、その上部に横並びでテンプレート選択ボタン（「頑張ってください！」「参考になりました」「詳細を教えてください」）が表示され、テンプレートボタンをタップするとそのテキストが入力欄に自動挿入される
...
1. **Given** 無料ユーザーがアプリを使用している、**When** プレミアムプラン案内画面を開く、**Then** 画面上部にプレミアムロゴ、続いて価格「月額680円」を大きく表示、その下に特典リスト（①AI機能無制限、②広告なし、③毎月200 DevCoin付与）を箇条書きで表示、最下部に「今すぐ加入」ボタンが配置される
...
- **外部SNS API障害時**: 利用可能なSNSの投稿のみを表示し、障害中のSNSには画面上部にオレンジ色の背景で通知バナー（例:「現在X(Twitter)からの投稿を取得できません」）を表示。バナーはタイムライン画面表示中は常時表示し、ユーザーが閉じるボタンをタップするまで表示を継続する
...
- **オフライン時の動作**: 最近閲覧したコンテンツ(過去24時間以内)をローカルキャッシュから表示。検索・新規投稿など新規取得が必要な操作を試みた際には、画面上部に黄色の背景色で「オフラインです。インターネット接続を確認してください」というスナックバーメッセージを3秒間表示
...
- **FR-002**: システムは、投稿された質問を一覧表示し、タイトル・投稿者・投稿日時・回答数・カテゴリタグを表示しなければならない。デフォルトのソート順は投稿日時の降順（最新が最上位）とする
...
- **FR-006**: システムは、特定のハッシュタグ（例：#個人開発チャレンジ）が付いた外部SNS（X/Threads/Instagram）の投稿を収集し、アプリ内タイムラインに時系列順で表示しなければならない。公開アカウントのみを対象とし、各SNSのAPI応答で`is_protected`（X）、`is_private`（Threads/Instagram）がfalseの投稿のみを取得する。ソート順は投稿日時の降順、同一時刻の場合は投稿ID（文字列）の昇順とする
...
- **FR-010**: システムは、質問・回答・SNS投稿にコメントを投稿できる機能を提供し、テンプレート化されたコメントを選択可能にしなければならない。テンプレートは以下の3種類: ①「頑張ってください！」（encouragement）、②「参考になりました」（helpful）、③「詳細を教えてください」（question）
- **FR-011**: システムは、他の開発者の投稿に「応援する」アクションを実行でき、投稿者に通知が届く機能を提供しなければならない。応援済みの投稿には、投稿右下に赤色のハートアイコン（24×24px）が表示され、応援数が数値で併記される
- **FR-012**: システムは、月額680円のプレミアムプランを提供し、特典（AI機能無制限/広告なし/月200 DevCoin付与）を適用しなければならない
- **FR-013**: システムは、プレミアムプラン加入時に3秒以内に特典を有効化し、サブスクリプションキャンセル後は請求期間終了まで特典を維持しなければならない
